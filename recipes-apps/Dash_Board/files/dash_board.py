...
class Ui_MainWindow(Worker):
    ...
    def main_task(self,MainWindow):
        ...
        while True:
            ...
            current_time = int(datetime.now().strftime("%M")) +(60*(current_hour-start_hour))
            Elapsed_time = current_time - start_time
            current_KM += ((speed_data*Elapsed_time)/60)
            self.set_KM(current_KM)

    def run_task(self,MainWindow):
            self.thread = QThread()
            self.worker = Ui_MainWindow()
            self.worker.moveToThread(self.thread)
            self.thread.started.connect(self.worker.main_task(MainWindow))
